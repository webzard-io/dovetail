...
export async function getResources(base: string): Promise<string[]> {
...
export async function getResourceSpec(def: string): Promise<any> {
  if (!cache.openApi.swagger) {
    cache.openApi = await ky.get("/proxy-k8s/openapi/v2").json<any>();
  }
  let spec = cache.openApi.definitions[def].properties.spec;
  resolveRef(spec);
  return spec;
}
